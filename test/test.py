"""
Tests for the geometry in _utils.py. Numpy only, no model, no GPU.
...
def test_gauge_removes_rigid_drift():
    ...
    traj, res = u.gauge_trajectory(bg, mem)
    check("residual is zero under pure rigid drift", res.max() < 1e-8, f"max {res.max():.2e}")
    check("gauged displacement is zero", u.displacement(traj).max() < 1e-8)
...
def test_coherence_separates_shared_from_private():
    ...
    hi = u.velocity_coherence(shared)["top"]
    lo = u.velocity_coherence(private)["top"]
    ...
    check("shared participation ratio is ~1", u.velocity_coherence(shared)["pr"].max() < 1.01)
...
def test_novel_fraction_sees_new_directions():
    ...
    fi = u.novel_fraction(inside, bg0, k = 16)[-1]
    fo = u.novel_fraction(outside, bg0, k = 16)[-1]
...
def test_trajectory_shape():
    ...
    check("straight line has tortuosity 1", abs(np.median(u.tortuosity(straight)) - 1.0) < 1e-6)
    check("wandering has tortuosity > 2", np.median(u.tortuosity(wander)) > 2.0,
          f"{np.median(u.tortuosity(wander)):.2f}")
    check("straight line keeps consecutive cosine 1", u.incremental_cosine(straight).min() > 0.999)
    check("wandering keeps consecutive cosine near 0", abs(u.incremental_cosine(wander)).max() < 0.3)
    check("straight line converges to final direction", np.nanmin(u.direction_to_final(straight)) > 0.999)
...
def test_separation_is_scale_free():
    ...
    seps = [u.cloud_separation(mem0 + a * d, fill0) for a in (0, 5, 20, 80)]
    ...
    check("a shift of one cloud radius reads ~1",
          abs(u.cloud_separation(mem0 + np.sqrt(D) * d, fill0) - 1.0) < 0.15,
          f"{u.cloud_separation(mem0 + np.sqrt(D) * d, fill0):.2f}")
    check("separation is invariant to global rescaling",
          abs(u.cloud_separation(2 * (mem0 + 5 * d), 2 * fill0) - seps[1]) < 1e-8)
...
def test_summarise_contract():
    ...
    s = u.summarise(mem, fill, bg0, k = 16)
...
def test_schedule():
    ...
    for total in (50, 400, 2000):
        sched = u.checkpoint_schedule(total)
